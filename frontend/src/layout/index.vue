<template>
	...
	<template v-if="siteThemeStore.frameLayout=='default'">
		...
		<section class="lee-panel-wrapper">
			<div v-if="!ismobile" :class="isMenuCollapse ? 'lee-panel-side isCollapse' : 'lee-panel-side'">
				<div class="lee-panel-side-scroll">
					<el-scrollbar>
				...
				<div class="lee-panel-side-bottom" @click="setMenuCollapsed">
					<el-icon>
			...
			<div class="lee-panel-body el-container">
				<TabsView v-if="ismultitabs" />
				<div class="lee-panel-main" id="lee-panel-main">
	...
	<template v-else-if="siteThemeStore.frameLayout=='header'">
		<header class="lee-panel-header">
			...
			<div v-if="!ismobile" class="lee-header-menu" style="margin-left: 20px;margin-right: 20px;">
	...
	<template v-else-if="siteThemeStore.frameLayout=='fenlan'">
		...
		<section class="lee-panel-wrapper">
			<div v-if="!ismobile" class="lee-panel-side-split">
				<div class="lee-panel-side-scroll">
					<el-scrollbar>
						<ul>
			...
			<div v-if="!ismobile && nextMenu.length>0" :class="isMenuCollapse ? 'lee-panel-side isCollapse' : 'lee-panel-side'">
				<div class="lee-panel-side-scroll">
					<el-scrollbar>
						<el-menu class="rymenu" :default-active="active" router :collapse="isMenuCollapse" :unique-opened="true" :collapse-transition="false">
	...
	<transition name="el-zoom-in-top">
		<ul v-show="contextMenuVisible" :style="{ left: left + 'px', top: top + 'px' }" class="contextmenu"
			id="leeleftMenuContext" ref="leeleftMenuContext">
...
// 点击其他地方时关闭右键菜单
let leeleftMenuContext = ref(null)
function handleClickOutside(event) {
	if (leeleftMenuContext.value && !leeleftMenuContext.value.contains(event.target)) {
		closeContextMenu();
	}
}
...
function openContextMenu(e) {
	...
	if (obj.role == 'menuitem') {
		...
		nextTick(() => {
			let ct = document.getElementById("leeleftMenuContext");
...
.lee-panel-side-scroll:deep(.el-menu-item).is-active{
	background-color: var(--el-color-primary-light-8) !important;
}
.lee-panel-side-scroll:deep(.el-menu-item.is-active)::before{
	position: absolute;
    border-radius: 4px;
    left: 12px;
    width: 4px;
