...
<template>
	<div class="lyTable" :style="{ height: _height }" ref="lyTableMain" v-loading="loading">
		<div class="leetopaction" v-if="!hideTopBar">
			<div class="leeflexcenter">
				<slot name="topbar"></slot>
				<el-button icon="Download" type="primary" @click="handleExport" title="导出" v-if="!hideExport"></el-button>
				<LeeImportFile ref="lyImportRef" :apiObj="apiImportObj" :apiTemplateObj="apiTemplateObj" @success="handleImportSuccess" style="margin-left: 12px;" v-auth="'Import'"></LeeImportFile>
			</div>
			<TableActions
				style="float: right;"
...
<script setup>
	...
	import LeeImportFile from '@/components/leeImportFile.vue'
