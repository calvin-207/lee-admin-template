...
from system.models import MenuField, FieldPermission
...
class FieldPermissionMixin:
    ...
    permission_cache_prefix = "leeadmin_template_field_perm:"
    ...
    def query_database_permissions(self, model_name, role_ids):
        ...
        queryset = FieldPermission.objects.filter(
            field__model=model_name, role__in=role_ids
        ).values(
            "can_create", "can_view", "can_update", field_name=F("field__field_name")
        )
            ...
            def __init__(self, *args, **kwargs):
                ...
                for field_name, field in list(fields.items()):
                    if field_name in permissions:
                        ...
                        # 控制写权限
                        if (
                            self.context.get("action") == "create"
                            and not perm["can_create"]
                        ):
                            field.read_only = True
                        elif (
                            self.context.get("action") in ["update", "partial_update"]
                            and not perm["can_update"]
                        ):
                            field.read_only = True
