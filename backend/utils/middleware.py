...
class ApiLoggingMiddleware(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if (
            self.enable
            and hasattr(view_func, "cls")
            and hasattr(view_func.cls, "queryset")
        ):
    ...
    def process_request(self, request):
        ...
        if IS_SINGLE_TOKEN and request.request_path[0:9] not in FRONTEND_API_LIST:
            ...
            if (
                jwt_token
                and "JWT" in jwt_token
                and jwt_token.split("JWT ")[1] != "null"
            ):
                error_data = {
                    "msg": "身份认证已经过期，请重新登入",
                    "code": 4001,
                ...
                try:
                    ...
                    cache_token = redis_conn.get(f"lee-single-token{user.id}")
...
class OperateAllowFrontendView(APIView):
    ...
    def get(self, request):
        ...
        return SuccessResponse(
            data={"is_allow": ApiLoggingMiddleware.IS_ALLOW_FRONTEND}, msg="success"
        )
