...
table_prefix = "leeadmin_"  # 数据库表名前缀
...
class BaseModel(models.Model):
    ...
    is_delete = models.BooleanField(
        default=False, verbose_name="是否逻辑删除", help_text="是否逻辑删除"
    )
...
def get_model_info_by_app(app_name, include_fields=True):
    ...
    model_classes = [
        cls
        for cls in model_module.__dict__.values()
...
def get_project_app_models(app_name=None, exclude_apps=None, include_fields=True):
    ...
    for app_config in apps.get_app_configs():
        if app_config.name.startswith("django") or app_config.name in exclude_apps:
            continue

        try:
            models = get_model_info_by_app(
                app_config.name, include_fields=include_fields
            )
