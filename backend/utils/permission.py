...
from config import (
    IS_DEMO,
    CUSTOM_PERMISSION_CAHCE,
    CUSTOM_PERMISSION_CAHCE_TIME,
    CUSTOM_PERMISSION_WHITELIST,
)
from system.models import RoleMenuButtonPermission
...
class APIPermissionValidator:
    ...
    def get_user_permissions(cls, user_id, role_ids):
        ...
        if APIPermissionValidator.CAN_CACHE:
            cache_key = f"leeadmin_template_user_permissions_{user_id}"
            permissions = cache.get(cache_key)

        if permissions is None:
            # 缓存未命中，从数据库查询
            permissions = set(
                RoleMenuButtonPermission.objects.filter(role__in=role_ids).values_list(
                    "menu_button__api", "menu_button__method"
                )
            )
            if APIPermissionValidator.CAN_CACHE:
                # 设置缓存
    ...
    def clear_user_permissions_cache(cls, user_id=None, is_all=False):
        ...
        if not is_all:
            ...
            keys = cache.keys("leeadmin_template_user_permissions_*")
            if keys:
...
class CustomPermission(BasePermission):
    ...
    def _get_db_whitelist(self):
        ...
        if whitelist is None:
            ...
            if wlist:
                ...
                if CUSTOM_PERMISSION_CAHCE:
                    cache.set(
                        cache_key, whitelist, timeout=CUSTOM_PERMISSION_CAHCE_TIME
                    )
            else:
                return []
    ...
    def _check_api_permission(self, request) -> bool:
        ...
        for perm_api, perm_method in user_permissions:
            if (
                current_method == perm_method
                and APIPermissionValidator.validate_api_path(current_api, perm_api)
            ):
                return True
