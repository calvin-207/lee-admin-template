...
def get_dept_children_ids(dept_id, include_self=False):
    ...
    if DATA_FILTER_CACHE:
        cache_key = f"leeadmin_template_dept_children:{dept_id}"
...
class DataLevelPermissionsFilter(BaseFilterBackend):
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        menu_button_data = MenuButton.objects.filter(
            api=re_api, method=method
        ).values_list("id", "menu_id", named=True)
        if menu_button_data:
        ...
        if menu_button_ids:
            ...
            role_permissions = RoleMenuButtonPermission.objects.filter(
                role__in=role_ids, menu_button_id__in=menu_button_ids
            ).values("data_scope")

        max_data_scope = self._get_max_data_scope(role_permissions)
        is_menu = False
        if max_data_scope in [
            5,
            -1,
        ]:  # 按钮数据权限未配置则按本菜单的全局数据权限策略处理
            is_menu = True
            if menu_ids:
                # 获取角色菜单权限数据（合并查询条件）
                role_menu_permissions = RoleMenuPermission.objects.filter(
                    role__in=role_ids, menu_id__in=menu_ids
                ).values("data_scope")
        ...
        dept_ids = self._get_permitted_dept_ids(
            user_dept_id, role_ids, max_data_scope, is_menu=is_menu
        )
    ...
    def _get_permitted_dept_ids(
        self, user_dept_id, role_ids, max_data_scope, is_menu=True
    ):
        """
        根据权限级别获取允许访问的部门ID列表
        is_menu:是否使用菜单的数据权限
        ...
        if max_data_scope == 1:  # 本部门
            ...
            if is_menu:
                custom_depts = RoleMenuPermission.objects.filter(
                    role__in=role_ids, data_scope=3
                ).values_list("dept__id", flat=True)
            else:
                custom_depts = RoleMenuButtonPermission.objects.filter(
                    role__in=role_ids, data_scope=3
                ).values_list("dept__id", flat=True)
...
class CustomDjangoFilterBackend(DjangoFilterBackend):
    ...
    def find_filter_lookups(self, orm_lookups, search_term_key):
        for lookup in orm_lookups:
            new_lookup = (
                LOOKUP_SEP.join(lookup.split(LOOKUP_SEP)[:-1])
                if len(lookup.split(LOOKUP_SEP)) > 1
                else lookup
            )
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if filterset.__class__.__name__ == "AutoFilterSet":
            ...
            filter_fields = (
                filterset.filters
                if self.filter_fields == "__all__"
                else self.filter_fields
            )
            orm_lookup_dict = dict(
                zip(
                    [field for field in filter_fields],
                    [
                        filterset.filters[lookup].lookup_expr
                        for lookup in filterset.filters.keys()
                    ],
                )
            )
            orm_lookups = [
                self.construct_search(lookup, lookup_expr)
                for lookup, lookup_expr in orm_lookup_dict.items()
            ]
            conditions = []
            queries = []
