...
class ImportExportMixin:
    ...
    default_file_name = (
        "ly" + time.strftime("%Y%m%d%H%M%S") + str(random.randint(10, 99)) + ".xlsx"
    )
    import_field_dict = {}
    export_field_dict = {}
    import_image_fields = []
    ...
    save_dir = os.path.join(
        "systemexport", time.strftime("%Y-%m-%d", time.localtime(time.time()))
    )
    # 保存位置
    ...
    def inserImg(self, sheet, Imgpath, row_index, col_index):
        ...
        try:
            ...
            resize_H = 40
            resize_W = 40
            imgsize_excel = XDRPositiveSize2D(
                pixels_to_EMU(resize_W), pixels_to_EMU(resize_H)
            )
    ...
    def process_nested_field(
        self, field_name, value, data_dict, is_many=False, separator=","
    ):
        """
        处理嵌套字段和多对多字段
    ...
    def export_data(self, request, *args, **kwargs):
        ...
        if not export_fields:
            assert self.export_field_dict, (
                "'%s' 请配置对应的导出模板字段。" % self.__class__.__name__
            )
        ...
        if not self.export_serializer_class:
            self.export_serializer_class = self.serializer_class
        assert self.export_serializer_class, (
            "'%s' 请配置对应的导出序列化器。" % self.__class__.__name__
        )
        ...
        try:
                ...
                for h_index, h_item in enumerate(field_order):
                    if h_index == 0:
                        ...
                        if field_name in self.import_image_fields or (
                            "." in field_name
                            and field_name.split(".")[-1] in self.import_image_fields
                        ):
                            if cell_value and self.is_image(cell_value):
                                try:
                                    self.inserImg(
                                        ws, cell_value, index + 2, h_index + 1
                                    )
                            ...
                            # 普通字段处理
                            self.cell_nomall_data(
                                ws, index, h_index, df_len_max, cell_value, left_center
                            )
            ...
            response["Content-Disposition"] = (
                f"attachment; filename={escape_uri_path(export_file_name)}"
            )
            return response
    ...
    def import_data(self, request, *args, **kwargs):
        ...
        assert self.import_field_dict, (
            "'%s' 请配置对应的导入模板字段。" % self.__class__.__name__
        )
        assert self.import_serializer_class, (
            "'%s' 请配置对应的导入序列化器。" % self.__class__.__name__
        )
                    ...
                    for idx, cell_value in enumerate(row):
                        ...
                        if excel_col_name in self.import_field_dict:
                            ...
                            if hasattr(self, "import_image_fields") and (
                                model_field in self.import_image_fields
                                or (
                                    "." in model_field
                                    and model_field.split(".")[-1]
                                    in self.import_image_fields
                                )
                            ):
                                cell_value = (
                                    cell_value
                            ...
                            self.process_nested_field(
                                model_field, cell_value, row_data, is_many, separator
                            )
    ...
    def generate_template(self):
        ...
        # 获取示例数据
        example_data = (
            self.get_import_example_data()
            if hasattr(self, "get_import_example_data")
            else {}
        )
    ...
    def download_template(self, request, *args, **kwargs):
        ...
        try:
            ...
            filename = (
                f"import_template_{mname}.xlsx"
                if mname
                else f"import_template_{time.strftime('%Y%m%d')}.xlsx"
            )
            response["Content-Disposition"] = (
