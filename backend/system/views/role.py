...
from system.views.menu_field import MenuFieldSerializer
...
class RoleCreateUpdateSerializer(CustomModelSerializer):
    ...
    key = serializers.CharField(
        max_length=50,
        validators=[
            UniqueValidator(queryset=Role.objects.all(), message="权限字符必须唯一")
        ],
    )
    name = serializers.CharField(
        max_length=50,
        validators=[
            UniqueValidator(queryset=Role.objects.all(), message="角色名称必须唯一")
        ],
    )
...
class RolePermissonSerializer(CustomModelSerializer):
    ...
    role_button_permission = RoleMenuButtonPermissionSerializer(
...
class RolePermissionViewSet(CustomModelViewSet):
    ...
    def save_permission(self, request, *args, **kwargs):
        ...
        if instance:
            ...
            RoleMenuButtonPermission_list = ast_convert(
                reqData.get("RoleMenuButtonPermission", [])
            )
            ...
            try:
                ...
                # 2. 批量创建新权限
                stats = {
                    "menu": self._bulk_create_menu_permissions(
                        RoleMenuPermission_list, request
                    ),
                    "button": self._bulk_create_button_permissions(
                        RoleMenuButtonPermission_list, request
                    ),
                    "field": self._bulk_create_field_permissions(
                        FieldPermission_list, request
                    ),
                }
    ...
    def _bulk_create_button_permissions(self, data, request):
        ...
        serializer = RoleMenuButtonPermissionSerializer(
            data=data, many=True, request=request
        )
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return len(serializer.validated_data)
    ...
    def _bulk_create_field_permissions(self, data, request):
        ...
        if not data:
            return 0
        serializer = FieldPermissionSerializer(data=data, many=True, request=request)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return len(serializer.validated_data)
