...
class SystemConfigUpdateSerializer(CustomModelSerializer):
    ...
    def validate_key(self, value):
        ...
        instance = (
            SystemConfig.objects.exclude(id=ins.id)
            .filter(key=value, parent__isnull=True)
            .exists()
        )
        if instance:
            raise ValueError("已存在相同变量名")
...
class SystemConfigViewSet(CustomModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(
            instance, data=body, request=request, partial=partial
        )
...
def getSystemConfig(key="base", group=True):
    ...
    """
    if group:
        queryset = SystemConfig.objects.filter(parent__key=key).values(
            "value", "key", "form_item_type"
        )
    else:
        keys = key.split(".")  # 拆分成 ["a", "b"]
        if len(keys) == 2:
            queryset = SystemConfig.objects.filter(
                parent__key=keys[0], key=keys[1]
            ).values("value", "key", "form_item_type")
        elif len(keys) == 1:
            queryset = SystemConfig.objects.filter(key=key).values(
                "value", "key", "form_item_type"
            )
        else:
