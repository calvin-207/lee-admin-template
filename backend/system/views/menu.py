...
from system.models import Menu, MenuButton, Button, RoleMenuPermission
...
class MenuTreeSerializer(CustomModelSerializer):
    ...
    def get_menu_buttons_name(self, instance):
        queryset = MenuButton.objects.filter(menu=instance.id).values_list(
            "name", flat=True
        )
        if queryset:
            return queryset
...
class MenuViewSet(CustomModelViewSet):
    ...
    def update_sort(self, request):
        ...
        if not menu_list:
            return ErrorResponse(msg="没有提供数据")
    ...
    def web_router(self, request):
        ...
        if user.is_superuser:
            ...
            queryset = self.filter_queryset(
                Menu.objects.filter(id__in=menuIds, status=True)
            )
