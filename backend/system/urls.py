...
from system.views.button import ButtonViewSet
...
from system.views.role import RoleViewSet, RolePermissionViewSet
...
from system.views.system_config import (
    SystemConfigViewSet,
    GetSystemConfigSettingsView,
)
from system.views.notification import NotificationViewSet
...
system_url.register(
    r"role_permission", RolePermissionViewSet, basename="role_permission"
)
...
urlpatterns = [
    ...
    re_path(
        "menu_button/menu_button_permission/",
        MenuButtonViewSet.as_view({"get": "menu_button_permission"}),
    ),
    re_path(
        "menu_button/batch_generate/",
        MenuButtonViewSet.as_view({"post": "batch_generate"}),
    ),
    re_path("menu_field/get_models/", MenuFieldViewSet.as_view({"get": "get_models"})),
    re_path(
        "menu_field/auto_create/", MenuFieldViewSet.as_view({"post": "auto_create"})
    ),
    ...
    re_path(
        "dictionary/set_status/", DictionaryViewSet.as_view({"post": "set_status"})
    ),
    re_path(
        "user/reset_password/(?P<pk>.*?)/",
        UserViewSet.as_view({"put": "reset_password"}),
    ),
    re_path(
        "role_id_to_menu/(?P<pk>.*?)/", RoleViewSet.as_view({"get": "roleId_to_menu"})
    ),
    re_path(
        "role_permission/save_permission/",
        RolePermissionViewSet.as_view({"post": "save_permission"}),
    ),
    re_path(
        "operation_log/deletealllogs/",
        OperationLogViewSet.as_view({"delete": "deletealllogs"}),
    ),
    path(
        "operation_log/getOwnerLogs/",
        OperationLogViewSet.as_view({"get": "getOwnerLogs"}),
    ),
    re_path(
        "login_log/deletealllogs/", LoginLogViewSet.as_view({"delete": "deletealllogs"})
    ),
    path("login_log/getOwnerLogs/", LoginLogViewSet.as_view({"get": "getOwnerLogs"})),
    path(
        "user/user_info/",
        UserViewSet.as_view({"get": "user_info", "put": "update_user_info"}),
    ),
    ...
    re_path(
        "sysconfig/save_content/(?P<pk>.*?)/",
        SystemConfigViewSet.as_view({"put": "save_content"}),
        name="保存配置",
    ),
    path(
        "sysconfig/get_models_info_list/",
        SystemConfigViewSet.as_view({"get": "get_models_info_list"}),
        name="获取所有models列表信息",
    ),
    path("getconfig/", GetSystemConfigSettingsView.as_view()),
    path(
        "msg/ownmsg/",
        NotificationViewSet.as_view({"get": "get_own_receive"}),
        name="获取自己的消息列表",
    ),
    path(
        "msg/delownmsg/",
        NotificationViewSet.as_view({"post": "del_own_receive"}),
        name="删除自己的消息",
    ),
    path(
        "msg/readownmsg/",
        NotificationViewSet.as_view({"post": "read_own_receive"}),
        name="设置自己的消息已读",
    ),
]
